<div id="change-password-user-modal" tabindex="-1" aria-hidden="true" class="hidden bg-black/30 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-0 p-4 w-full md:inset-0 h-modal md:h-full">
    <div class="relative w-full max-w-md h-full md:h-auto top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/3 z-50">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="pt-4 pb-8 px-6 lg:px-8">
                <form class="flex flex-col gap-4" method="POST" action="{{route('user.update_password')}}">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="old_password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Old Password</label>
                        <input type="password" name="old_password" placeholder="••••••••" id="old_password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                    </div>
                    <div>
                        <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                        <input type="password" name="new_password" id="password" placeholder="••••••••" class="bg-gray-50 disabled:bg-red-50 border border-gray-300 disabled:border-red-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white " required>
                    </div>
                    <div>
